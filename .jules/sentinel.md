...
## 2026-04-24 - [Preventing CRLF Log Injection via Unsanitized Request Data]
**Vulnerability:** The application was directly interpolating user-controlled data (`req.originalUrl` and the `origin` header) into `console.warn` logging statements without sanitization. If an attacker sent a request with URL-encoded carriage returns (`%0D`) and line feeds (`%0A`), these characters could be interpreted by the logging system, allowing the attacker to inject entirely fake log entries or disguise malicious activity.
**Learning:** Any data originating from the client (URLs, Headers, User Agents) must be treated as untrusted. Directly writing this data to logs creates an injection vector known as Log Forging or CRLF Injection. Even if the application logic correctly rejects the malicious request, the very act of logging the rejection using unsanitized input is a vulnerability.
**Prevention:** Sanitize all user-controlled input before logging. A common and effective approach is to strip or replace newline characters (`\r` and `\n`) from the input string (e.g., `String(input).replace(/[\r\n]/g, '')`) before passing it to the logging function.
