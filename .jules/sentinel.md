...
## 2024-04-28 - [Preventing CRLF Log Injection via Unsanitized Headers/Paths]
...
## 2026-04-20 - [Missing Content Security Policy (CSP) in Statically Served Frontend]
**Vulnerability:** The application was missing a Content Security Policy (CSP). While the Express backend used `helmet` to set security headers, these headers only applied to API responses (`/api/dashboard`), not the frontend HTML, because the frontend is served independently via Nginx (in production) or Vite (in development). This left the application vulnerable to Cross-Site Scripting (XSS) and data exfiltration attacks.
**Learning:** Security headers configured in a backend server (e.g., via `helmet`) do not automatically protect a decoupled frontend application. If the HTML is served statically by a different server or reverse proxy, the CSP and other security headers must be explicitly defined there (e.g., via a `<meta>` tag in `index.html` or in the Nginx configuration).
**Prevention:** Always implement a strict Content Security Policy for the frontend application, either via a `<meta http-equiv="Content-Security-Policy">` tag in the entry HTML file or by configuring the static file server (like Nginx) to inject the appropriate headers.

## 2026-04-21 - [Preventing Memory Exhaustion via Unbounded JSON Payloads]
...
## 2024-06-01 - [Missing Security Headers in Nginx configuration]
**Vulnerability:** The statically served frontend via Nginx did not set standard security headers (X-Frame-Options, X-Content-Type-Options, Referrer-Policy) and leaked the server version (`server_tokens off;` was missing). The `helmet` middleware in the Express backend doesn't apply to the frontend HTML served by Nginx.
**Learning:** For decoupled architectures where the frontend is served statically via a reverse proxy like Nginx, security middlewares like `helmet` in the backend only protect API responses. The static assets and HTML must be protected by explicit header configuration in the proxy/server delivering them.
**Prevention:** Always configure essential security headers (`X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`) and disable server tokens (`server_tokens off;`) directly in the `nginx.conf` file or corresponding web server configuration to protect the frontend.
