...
**Vulnerability:** The Express backend used the default `express.json()` middleware without specifying a maximum payload size. An attacker could send massive JSON payloads, forcing the server to buffer and parse them, potentially leading to memory exhaustion and Denial of Service (DoS).
**Learning:** Default configurations in body-parsing middlewares often lack strict limits, prioritizing functionality over security. Leaving payload sizes unbounded exposes the application to resource exhaustion attacks, especially on public-facing APIs.
**Prevention:** Always enforce strict payload size limits on Express body parsers (e.g., `express.json({ limit: '10kb' })`) and explicitly handle the resulting `entity.too.large` errors to return a safe `413 Payload Too Large` JSON response without leaking stack traces.
